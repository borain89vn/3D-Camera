 ...
 */
package edu.dhbw.andar.graphics;
...
import edu.dhbw.andar.models.Group;
import edu.dhbw.andar.models.Material;
...
public class Model3D extends ARObject implements Serializable{
	...
	// get X,Y model3d
	public  float  xModel3D;
	public  float yModel3D;
	//public float xRandom;
	//public  float yRandom;
	// get model3d id with x,y coordinate
	public static Hashtable<Float,Integer>  hashID;
	//public static Hashtable<Float,Integer>   hashYCoord;

	public int id;
	public boolean checkOnModel=true;
	public boolean checkOnBrick=false;
	public Model3D(Model model,String name,float x,float y) {
		super("model", name, 80.0, new double[]{0+x,0+y});
		this.model = model;
		hashID=new Hashtable<Float, Integer>();
		//hashYCoord=new Hashtable<Float,Integer>();
		 xModel3D=x;
		 yModel3D=y;
		model.finalize();
	...
	// get id model with x,y coordinate in screen
	
	public PointXY convertModelToPoint()
	{
		PointXY a=new PointXY(this.xModel3D,this.yModel3D);
		return a;
	}
	
	public int getIdModel(float xCoord,float yCoord)
	{
		return hashID.get(xCoord+yCoord);
	}
	@Override
